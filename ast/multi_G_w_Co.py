...
for n in range(list_size):
	...
	bound =  ([sub1,0.98,0.98,0,0,sub2,sub1,0.98,0],[sup1,1.02,1.02,float('inf'),float('inf'),sup2,sig*2,1.02,float('inf')])
	guess = [sig,1,1,1,1,sig,sig,1,0.3]
	...
	while(1):
		...
		chi2 = np.sum((fGs(fit_xlist,params[0],params[1],params[2],params[3],params[4],params[5],params[6],params[7],params[8]) - fit_ylist)**2)/np.size(fit_xlist)

		print('chi2: %f' %chi2)


		y7 = (y0 - y1) / (x0-x1) * (cut_xlist - x0) + y0
		ysimu = fGs(cut_xlist,params[0],params[1],params[2],params[3],params[4],params[5],params[6],params[7],params[8]) - y7
		...
		y8 = fG(cut_xlist,6855*params[6],6855*params[7],params[8])
		...
		FWHM_Co = params[6] * 2 * 300000 * np.sqrt(2*np.log(2))
		vshift_Fe = (params[1] - 1) * 300000 
		vshift_Ni = (params[2] - 1) * 300000
		vshift_Co = (params[7] - 1) * 300000
