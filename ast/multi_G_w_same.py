...
for n in range(list_size):
	...
	if g == 1:
		...
		Up_FWHM_Ni = FWHM_Ni - params[4] * 2 * 300000 * np.sqrt(2*np.log(2))
		Up_vshift_Ni = vshift_Ni - (params[1] - 1) * 300000
		Up_flux_ratio = flux_ratio - params[3] * params[4] * 7378 / params[2] / params[0] / 7155
		Up_high_ratio = high_ratio - params[3]/params[2]
		#edge
		vshift_Fe = [vshift_Fe]
		vshift_Ni = [vshift_Ni]
		FWHM_Fe = [FWHM_Fe]
		...
		while(k < 1000):
			...
			if np.abs(f_ratio/flux_ratio[0]) < 2 and np.abs(f_ratio/flux_ratio[0]) > 0.1 and np.abs(h_ratio/high_ratio[0]) < 3 and np.abs(h_ratio/high_ratio[0]) > 0.1:
			'''	
			f_ratio = params[3] * params[4] * 7378 / params[2] / params[0] / 7155
			h_ratio = params[3] / params[2]
			FWHM_Nit = params[4] * 2 * 300000 * np.sqrt(2*np.log(2))
			if f_ratio < 2 and f_ratio > 0.01 and h_ratio < 5 and FWHM_Nit < 12500:
				FWHM_Fe.append(params[0] * 2 * 300000 * np.sqrt(2*np.log(2)))
				FWHM_Ni.append(params[4] * 2 * 300000 * np.sqrt(2*np.log(2)))
				vshift_Fe.append((params[1] - 1) * 300000) 
				vshift_Ni.append((params[1] - 1) * 300000)
				flux_ratio.append(params[3] * params[4] *7378 / params[2] / params[0] / 7155)
				high_ratio.append(params[3]/params[2])
				k += 1
