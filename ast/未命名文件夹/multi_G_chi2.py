...
while(1):
	...
	collection = collections.BrokenBarHCollection.span_where(np.linspace(6800,7800,100),ymin=0,ymax=0.4,where=np.ones(100)>0,facecolor='gray',alpha=0.5)
	ax.add_collection(collection)

	ax.text(6600,0.45,'[Fe II] + [Ni II]')
	...
	Min3 = int(input('input the minimum3:'))
	Max3 = int(input('input the maximum3:'))
	...
	while(1):
		...
		print(xlist[pos[0]], xlist[pos[1]], xlist[pos[2]], xlist[pos[3]], xlist[pos[4]], xlist[pos[5]])
		cut_xlist = xlist[pos[0]:(pos[5]+1)]
		if width != 1:
			ylist_t = signal.savgol_filter(ylist,width,1)
		if width == 1:
			ylist_t = np.array(ylist)
		cut_ylist = ylist_t[pos[0]:(pos[5]+1)]
		print(len(cut_ylist))
		err_bar = np.abs(ylist[pos[0]:(pos[5]+1)] - cut_ylist)
		...
		x0 = cut_xlist[0]
		y0 = cut_ylist_f[0]
		x1 = cut_xlist[pos[5] - pos[0]]
		y1 = cut_ylist_f[pos[5] - pos[0]]
		...
		fit_err_bar = Append(err_bar[0:(pos[1] - pos[0]+1)],err_bar[(pos[2] - pos[0]):(pos[3] - pos[0]+1)])
		fit_err_bar = Append(fit_err_bar,err_bar[(pos[4] - pos[0]):(pos[5] - pos[0]+1)])
		...
		params,params_covariance=optimize.curve_fit(fGs,fit_xlist,fit_ylist,guess,sigma=fit_err_bar,maxfev=500000,bounds=bound)

		chi2 = np.sum(((fGs(fit_xlist,params[0],params[1],params[2],params[3],params[4],params[5]) - fit_ylist)/fit_err_bar)**2)
		chi2 = np.sqrt(chi2)/np.size(fit_xlist)
		print('chi2: %f' %chi2)
		...
		plt.yticks([])
		plt.plot(xlist[(pos[0]-100):(pos[5]+100+1)], ylist[(pos[0]-100):(pos[5]+100+1)], color="blue", label="data")
		...
		plt.plot(cut_xlist[(pos[4] - pos[0]):(pos[5] - pos[0]+1)], np.zeros(pos[5]-pos[4]+1)*min(cut_ylist_f), c = 'y')
	...
	if g == 1:
		...
		k = 0
		edge_size = float(input('edge size: '))
		while(k < 1000):
			print(k)
			...
			if (Min3 - Max2) >= 10:
				...
				redt2 = Max2
				bluet3 = Min3
			redt3 = np.random.rand()*edge_size*2 - edge_size + Max3
			n_min = 1
			indicator = 0
			...
			f_ratio = params[4] * params[5] / params[3] / params[0]
			h_ratio = params[4] / params[3]
			FWHM_Nit = params[5] * 2 * 300000 / 7378 * np.sqrt(2*np.log(2))
		...
		with open('/Users/pro/python/ast/Uncertenty'+save_as,'a') as f:
			f.writelines('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n' %(target_name, phase, U_vshift_Fe, U_vshift_Ni, U_FWHM_Fe, U_FWHM_Ni, U_flux_ratio, Min1, Max1, Min2, Max2, Min3, Max3, width, edge_size, chi2))
		g = 0
