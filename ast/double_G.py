...
while(1):	
	...
	while(1):
		...
		def fGs(x,s1,t1,t2,a,b,s2):
			return fG(x,s1,7155*t1,a)+fG(x,s2,7378*t2,b)+ (y0 - y1) / (x0-x1) * (x - x0) + y0
		...
		def fGs_improve(x, s1, t1, a):
			return fG(x,s1,7155*t1,a)

		ylist_tempt1 = cut_ylist_f - y7 - y5
		ylist_tempt2 = cut_ylist_f - y7 - y1
		...
		plt.plot(cut_xlist, y5, label = 'fit', c = 'r')
		plt.legend(loc = 'upper left')
		plt.show()
		...
		plt.plot(cut_xlist, fGs_improve(cut_xlist, params[0], params[1], params[3])+y5, label = 'fit_f', c = 'black')
		...
		plt.ylabel('Normalized flux')
		plt.yticks([])
		plt.plot(xlist[(pos[0]-100):(pos[5]+100+1)], ylist_t[(pos[0]-100):(pos[5]+100+1)], color="blue", label="data")
	...
	if g == 1:	
		...
		while(k < 1000):
			...
			'''
			'''
			6900.088601501541 7140.394012571646 7270.266472254973 7800.096474975807
			if np.abs(f_ratio/flux_ratio[0]) < 2 and np.abs(f_ratio/flux_ratio[0]) > 0.1 and np.abs(h_ratio/high_ratio[0]) < 3 and np.abs(h_ratio/high_ratio[0]) > 0.1:
			'''	
		...
		with open('/Users/pro/python/ast/Uncertenty'+save_as,'a') as f:
			f.writelines('%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n' %(target_name, phase, U_vshift_Fe, U_vshift_Ni, U_FWHM_Fe, U_FWHM_Ni, U_flux_ratio, Min1, Max1, Min2, Max2, Min3, Max3, width, edge_size))
		g = 0
